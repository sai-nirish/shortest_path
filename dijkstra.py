...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        nodeDic = {}
        path = None
        parentDic = {source : None}
        for node in nodes :
            nodeDistancePair = NodeDistancePair(node, float("inf"))
            nodeDic[node] = nodeDistancePair
            if node == source:
                nodeDistancePair.distance = 0
            queue.insert(nodeDistancePair)
        while(len(queue) > 0):
            nodeDistancePair = queue.extract_min()
            numVisited+=1
            if nodeDistancePair.node == destination :
                break
            for adj in nodeDistancePair.node.adj:
                # edge = (nodeDistancePair.node, adj)
                nodeDistancePair2 = nodeDic[adj]
                wei = weight(nodeDistancePair.node, adj)
                if nodeDistancePair2.distance > nodeDistancePair.distance + wei :
                    nodeDistancePair2.distance = nodeDistancePair.distance + wei
                    parentDic[adj] = nodeDistancePair.node 
                    queue.decrease_key(nodeDistancePair2)

        print(numVisited)

        if destination in parentDic:
            path = [destination]
            node = parentDic[destination]
